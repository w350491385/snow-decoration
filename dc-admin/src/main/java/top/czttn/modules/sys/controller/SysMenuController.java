...
import top.czttn.modules.sys.service.ShiroService;
...
public class SysMenuController extends AbstractController {
    ...
    public List<SysMenuEntity> list() {
        if (getUserId() != Constant.SUPER_ADMIN) {
            return sysMenuService.findListByCpId(getCpId());
        } else {
            List<SysMenuEntity> allList = sysMenuService.findAllList();
            return allList;
        }
    }
    ...
    public R delete(@PathVariable("menuId") long menuId) {
        //判断是否有子菜单或按钮
        List<SysMenuEntity> menuList = sysMenuService.queryListParentId(menuId,false);
        if (menuList.size() > 0) {
